...
export class OwnersService {
  ...
  getOwners(
    ...
    return {
      ...
      hasNext:
        this.owners.length > parseInt(pageSize, 10) * parseInt(page, 10),
    };
  }
  ...
  getOwnerPets(id: string): PetsAPI {
    let filter = `${id}`;
    return this.petService.getPets(filter);
  }
  ...
  private filter(search?: string) {
    return search
      ? Utils.filterByAll(search, this.owners)
      : this.owners;
  }
